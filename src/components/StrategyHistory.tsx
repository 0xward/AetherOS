...
import {
  ...
  RefreshCw, Crown, Clock, Wallet
} from 'lucide-react';
import { useStacks } from '@/components/providers/StacksProvider';
import { getStrategyHistory, deleteStrategy, type StrategyRecord } from '@/lib/strategyHistory';
...
export default function StrategyHistory() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(`Failed to load history: ${errMsg.includes('index') ? 'Firestore index needed — check Firebase Console for a link to create it.' : errMsg}`);
      console.error(err);
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative max-w-4xl mx-auto px-6">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/30 text-sm">Loading your strategy history...</p>
          </div>
        ) : records.length === 0 ? (
